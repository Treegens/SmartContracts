 ...
 */
contract TreegenNFT is ONFT721Enumerable, IERC4906, ReentrancyGuard {
     ...
     * @param _management The management contract address authorized to mint NFTs
    ...
    constructor(
        ...
        if (_management == address(0)) revert InvalidAddress();
    ...
    /**
     * @notice Updates the management contract address
     * @param _address The new management contract address
     * @dev Only callable by the contract owner
     */
    ...
    /**
     * @notice Safely mints a new NFT with reentrancy protection
     * @param to The address to mint the NFT to
     * @param tokenId The ID of the token to mint
     * @dev Only callable by the management contract. Uses Checks-Effects-Interactions pattern
     */
    function safeMint(address to, uint256 tokenId) public onlyManagement nonReentrant {
        if (to == address(0)) revert InvalidAddress();
        
        // Effects: Set URI before external call to prevent reentrancy
        string memory uri = string.concat(_defaultURI, Strings.toString(tokenId));
