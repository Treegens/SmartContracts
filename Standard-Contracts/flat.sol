...
contract MyToken is ERC721, ERC721URIStorage, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/";
    }
    ...
    function updateImg(uint8 _minted, uint8 _burnt, uint tokenId) external {
        if(_minted == _burnt){
            _setTokenURI(tokenId, "2.json");
        } else if(_minted>_burnt ){
            _setTokenURI(tokenId, "3.json");
        } else {
            _setTokenURI(tokenId, "4.json");
