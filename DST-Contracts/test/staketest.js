...
describe("SimpleStaking", () => {
  ...
  beforeEach(async () => {
    ...
    SimpleStaking = await SimpleStakingFactory.connect(owner).deploy(
      Token.address,
      owner.address,
      mgroVerification.address
    );
  ...
  it("should allow slashing by DAO", async () => {
    ...
    const initialBalance = await SimpleStaking.getStakedBalance(
      staker1.address
    );
    ...
    expect(finalBalance).to.equal(
      initialBalance.sub(expectedSlashAmount),
      "Incorrect staked balance after slashing"
    );
  });
  ...
  it("should not allow slashing when slashing is disabled", async () => {
    ...
    await expect(SimpleStaking.connect(mgroVerification).slash(staker1.address))
      .to.be.reverted;

    const finalBalance = await SimpleStaking.getStakedBalance(staker1.address);
    expect(finalBalance).to.equal(
      initialBalance,
      "Staked balance should remain unchanged"
    );
  });

  it("should only allow DAO to set slashing parameters", async () => {
    await expect(
      SimpleStaking.connect(staker1).setSlashingParams(slashingPercentage)
    ).to.be.reverted;
  });

  it("should only allow DAO to enable/disable slashing", async () => {
    await expect(SimpleStaking.connect(staker1).setSlashingEnabled(true)).to.be
      .reverted;
  });

  it("should revert when trying to stake without approving allowance", async () => {
    await expect(SimpleStaking.connect(staker1).stake(stakeAmount)).to.be
      .reverted;
  });
  ...
  it("should revert when trying to unstake more than the staked balance", async () => {
    ...
    expect(SimpleStaking.connect(staker1).unstake(stakeAmount.mul(2))).to.be
      .reverted;
  });

  it("should not allow slashing with zero slashing percentage", async () => {
    await expect(SimpleStaking.connect(owner).setSlashingParams(0)).to.be
      .reverted;
  });

  it("should not allow slashing a staker with zero balance", async () => {
    await expect(SimpleStaking.connect(owner).slash(staker1.address)).to.be
      .reverted;
  });
  ...
  it("should update lastStakedTime correctly on multiple stakes", async () => {
    ...
    // First stake
    await SimpleStaking.connect(staker1).stake(stakeAmount);
    const firstStakeTime = await SimpleStaking.getLastStakedTime(
      staker1.address
    );
    ...
    // Second stake
    await SimpleStaking.connect(staker1).stake(stakeAmount);
    const secondStakeTime = await SimpleStaking.getLastStakedTime(
      staker1.address
    );
