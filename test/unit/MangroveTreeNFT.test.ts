...
import type { MangroveTreeNFT } from '../../typechain-types';
...
describe('MangroveTreeNFT', function () {
  let nft: MangroveTreeNFT;
  ...
  beforeEach(async function () {
    ...
    const factory = await ethers.getContractFactory('MangroveTreeNFT');
    nft = (await factory.deploy()) as MangroveTreeNFT;
  ...
  describe('Deployment', function () {
    it('sets name and symbol', async function () {
      expect(await nft.name()).to.equal('Treegens Mangrove Planting');
      expect(await nft.symbol()).to.equal('TGMANGROVE');
    });
