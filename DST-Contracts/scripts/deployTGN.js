...
async function main() {
  ...
  const slasher = process.env.TGN_SLASHER || deployer.address;
  const treasury = process.env.TGN_TREASURY || deployer.address;
  const slashingPercent = Number(process.env.TGN_SLASHING_PERCENT || 10);

  if (
    !Number.isInteger(slashingPercent) ||
    slashingPercent < 1 ||
    slashingPercent > 30
  ) {
    throw new Error("TGN_SLASHING_PERCENT must be an integer between 1 and 30");
  }

  const Vault = await hre.ethers.getContractFactory("TGNVault");
  const TGNVault = await Vault.deploy(
    TGNToken.address,
    slasher,
    slashingPercent,
    treasury
  ...
  console.log("Vault Slasher: ", slasher);
  console.log("Vault Treasury: ", treasury);
  console.log("Vault Slashing Percent: ", slashingPercent);
