...
contract TGNVault is Ownable {
    ...
    // —— Constructor ——
    constructor(address _tgn, address _slasher, uint8 _slashingPercent, address _treasury) {
        if (_tgn == address(0) || _slasher == address(0) || _treasury == address(0)) revert InvalidInput();
        if (_slashingPercent == 0 || _slashingPercent > 30) revert InvalidInput();
        tgn = IERC20(_tgn);
        slasher = _slasher;
        treasury = _treasury;
