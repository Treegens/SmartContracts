...
contract MGROVerification is ReentrancyGuard, Ownable {
    event VerificationProposalCreated(
    ...
    event VerificationProposalExecuted(
        uint256 indexed proposalId,
        bool succeeded
    );
    ...
    modifier votingQuota() {
        require(
            maxOpenVotes[msg.sender] < MAX_PARALLEL_VOTES,
            "Too many open Votes"
        );
        _;
    }
    ...
    modifier onlyStakedMember() {
        require(
            tgnVault.getStakedBalance(msg.sender) > 0,
            "Not a staked member"
        );
        _;
    }
    ...
    function proposeVerification(uint256 _amount) external onlyStakedMember {
        ...
        VerificationProposal storage newProposal = verificationProposals[
            newProposalId
        ];
    ...
    function vote(
        uint256 _proposalId,
        bool _vote
    ) external onlyStakedMember votingQuota nonReentrant {
        VerificationProposal storage proposal = verificationProposals[
            _proposalId
        ];
        ...
        require(
            ...
            "User Blacklisted: Too many wrong verifications"
        );
    ...
    function executeVerification(
        uint256 _proposalId
    ) external payable onlyStakedMember nonReentrant {
        VerificationProposal storage proposal = verificationProposals[
            _proposalId
        ];
        ...
        if (proposalSucceeded) {
            ...
            mgmt.mintMgroTokens{value: msg.value}(recepient, amount);
