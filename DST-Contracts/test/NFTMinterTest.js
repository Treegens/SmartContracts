...
describe("TGN Test", function () {
  ...
  describe("Token Test", function () {
    it("Should allow owner to mint tokens", async function () {
      await TGN.mintWithTimelock(
        owner.address,
        ethers.utils.parseEther("10"),
        2705490014
      );

      expect(await TGN.balanceOf(owner.address)).to.equal(
        ethers.utils.parseEther("10")
      );
    });
    it("Users cannot transfer tokens if lock time is yet to elapse", async function () {
      ...
      await expect(
        TGN.transfer(user1.address, ethers.utils.parseEther("5"))
      ).to.be.revertedWith("Cannot transfer tokens till unlock time");
      await expect(
        TGN.transferFrom(
          owner.address,
          user1.address,
    ...
    it("Users can approve token usage by another contract", async function () {
      ...
      expect(await TGN.allowance(owner.address, add)).to.equal(
    ...
    it("Owner can mint 300M Tokens max", async function () {
      ...
      expect(await TGN.totalSupply()).to.equal(
        ethers.utils.parseEther("50000000")
      );
      await TGN.mintWithTimelock(
        add,
      ...
      await expect(
        TGN.mintWithTimelock(add, ethers.utils.parseEther("1"), 2705490014)
      ).to.be.revertedWith("Exceeds max supply");
    });

    it("Should only allow the owner to Mint tokens", async function () {
      await expect(
        TGN.connect(owner).mint(user1.address, ethers.utils.parseEther("10"))
      ).to.not.be.reverted;
      await expect(
        TGN.connect(owner).mintWithTimelock(
          user1.address,
          ethers.utils.parseEther("10"),
          2705490045
        )
      ...
      await expect(
        TGN.connect(user1).mint(user1.address, ethers.utils.parseEther("10"))
      ).to.be.reverted;
    });
  });
});
