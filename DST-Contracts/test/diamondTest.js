...
describe("DiamondTest", async function () {
  ...
  before(async function () {
    ...
    await managementFacet.initialize(
      NFTAddress,
      tokenAddress,
      owner.address,
      MGRADD.address
    ...
    // Add base URIs
    await managementFacet.addBaseURI(
      "ipfs://QmW3h5dB7yKyacNDfo1XCjjWV5zFyeDZfeVYcpYbx1xuNP/"
    );
    await managementFacet.addBaseURI(
      "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/"
    );
    await managementFacet.addBaseURI(
      "ipfs://QmaHhmm9bwJSF95NDwqyFiCX3LPDi7g6vY2zNXxQuDqgXe/"
    );
  });
  ...
  describe("ERC20 Token Minting and Burning", function () {
    ...
    it("The ERC20 Contract should only allow minting if it is called by the management contract", async function () {
      expect(await MGRADD.balanceOf(owner.address)).to.equal(
        ethers.utils.parseEther("10")
      );
      await expect(managementFacet.mintTokens(owner.address, 10)).to.not.be
        .reverted;
      await expect(
        MGRADD.mintTokens(owner.address, ethers.utils.parseEther("10"))
    ...
    it("User Should be able to burn tokens from the management contract", async function () {
      ...
      console.log(
        ...
        BigInt(await MGRADD.balanceOf(owner.address))
      );
      expect(await MGRADD.balanceOf(owner.address)).to.equal(
        ethers.utils.parseEther("20")
      );
    ...
    it("Should not change the stats values if the tokens are transferred", async function () {
      ...
      expect(await MGRADD.balanceOf(user1.address)).to.equal(
    ...
    it("Users can burn tokens only through the management contract", async function () {
      ...
      await expect(
        MGRADD.burnTokens(owner.address, ethers.utils.parseEther("5"))
      ).to.be.revertedWith("Unauthorized");
    });
  });
  ...
  describe("Base URI Management", function () {
    ...
    it("Should not allow more than 3 URIs", async function () {
      await expect(
        managementFacet.addBaseURI(
          "ipfs://QmaHhmm9bwJSF95NDwqyFiCX3LPDi7g6vY2zfhjdfgnXe"
        )
      ).to.be.revertedWith("Cannot have more than 3 URIs");
    });
  });
  ...
  describe("NFT Minting and URI Updates", function () {
    ...
    it("Check for the Minted NFT to be set to the baseURI[0] on mint", async function () {
      expect(await NFTADD.tokenURI(1)).to.equal(
        "ipfs://QmW3h5dB7yKyacNDfo1XCjjWV5zFyeDZfeVYcpYbx1xuNP"
      );
    });
    ...
    it("Users should be able to mint NFTs, and tokenId added to array of owned tokens", async function () {
      ...
      console.log(
        "User holds: ",
        await managementFacet.checkStats(owner.address)
      );
    });
    ...
    it("Should update the URI if the minted is greater than burnt", async function () {
      ...
      await managementFacet.mintTokens(
        owner.address,
      ...
      expect(await NFTADD.tokenURI(1)).to.equal(
        "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/2/1"
      );
    });
    ...
    it("Should update the URI if the minted is greater than burnt and minted is greater than 100", async function () {
      ...
      await managementFacet.updateNFTs(owner.address);
      expect(await NFTADD.tokenURI(1)).to.equal(
        "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/2/2"
      );
    });
    ...
    it("Should update the URI if the minted is greater than burnt and minted is greater than 150", async function () {
      ...
      await managementFacet.updateNFTs(owner.address);
      expect(await NFTADD.tokenURI(1)).to.equal(
        "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/2/3"
      );
      expect(await NFTADD.tokenURI(2)).to.equal(
        "ipfs://Qmbza7VprgNZ8eWzjRFWBaZUj11tZ2kEHVA6VUZGnsGVtu/2/3"
      );
    });
    ...
    it("Should update the URI if the burnt is greater than minted", async function () {
      ...
      await managementFacet
        .connect(user1)
        .burnTokens(ethers.utils.parseEther("10"));
      ...
      expect(await NFTADD.tokenURI(3)).to.equal(
        "ipfs://QmaHhmm9bwJSF95NDwqyFiCX3LPDi7g6vY2zNXxQuDqgXe/2/1"
      );
    });
  });
});
